...
crossover_data = {}
user_status = {}
CHOOSING_NAME, CHOOSING_DURATION, CHOOSING_INTRO, CHOOSING_OBJECTIVE, CHOOSING_PHOTO = range(5)
MODIFY_SELECT, MODIFY_INPUT = range(5, 7)

def save_data():
    with open(DATA_FILE, "w") as f:
        json.dump({"crossover": crossover_data, "users": user_status}, f)
...
def load_data():
    if os.path.exists(DATA_FILE):
        with open(DATA_FILE, "r") as f:
            try:
                data = json.load(f)
                crossover_data.update(data.get("crossover", {}))
                user_status.update(data.get("users", {}))
            except Exception as e:
                print("Erreur chargement:", e)

async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("👋 Bienvenue ! Tape /crossover pour voir le crossover en cours.")

async def crossover_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def handle_join(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_status[username] = {"status": "joined", "seen": True}
    save_data()
    await query.message.delete()
    await query.message.chat.send_message("✅ Tu as rejoint le crossover !")
    await query.answer()

async def handle_ignore(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    username = query.from_user.username or f"id_{query.from_user.id}"
    user_status[username] = {"status": "ignored", "seen": True}
    save_data()
    await query.message.delete()
    await query.message.chat.send_message("🚫 Tu as ignoré le crossover.")
    await query.answer()
...
async def participants_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    joined = [f"@{u}" for u, v in user_status.items() if v.get("status") == "joined"]
    if not joined:
        await update.message.reply_text("👥 Aucun participant pour l’instant.")
    else:
...
async def join_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_status[username] = {"status": "joined", "seen": True}
    save_data()
    await update.message.reply_text("✅ Tu as rejoint le crossover.")
...
async def leave_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_status[username] = {"status": "ignored", "seen": True}
    save_data()
    await update.message.reply_text("🚫 Tu as quitté le crossover.")
    
async def crossover_now_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.chat.type != "private" or update.message.from_user.username != CREATOR_USERNAME:
        await update.message.reply_text("❗ Commande réservée au créateur, en privé.")
...
async def set_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("⏳ Quelle est sa *durée (en jours)* ?")
...
async def set_duration(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text("📝 Donne une *intro stylée* pour ce crossover.")
        return CHOOSING_INTRO
    except:
        await update.message.reply_text("❗ Envoie un nombre valide.")
        return CHOOSING_DURATION

async def set_intro(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data["intro"] = update.message.text
    await update.message.reply_text("🎯 Quel est l’*objectif* du crossover ?")
    return CHOOSING_OBJECTIVE

async def set_objective(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data["objective"] = update.message.text
    await update.message.reply_text("📸 Envoie une *photo* ou un *fichier image*.")
    return CHOOSING_PHOTO
...
async def set_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not file_id:
        await update.message.reply_text("❗ Merci d’envoyer une image.")
        return CHOOSING_PHOTO
...
async def modify_crossover(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.chat.type != "private" or update.message.from_user.username != CREATOR_USERNAME:
        await update.message.reply_text("❗ Commande réservée au créateur en privé.")
        return ConversationHandler.END
    if not crossover_data:
        await update.message.reply_text("⚠️ Aucun crossover à modifier.")
        return ConversationHandler.END

    await update.message.reply_text("✏️ Que veux-tu modifier ? (nom, intro, objectif, durée)")
    return MODIFY_SELECT

async def modify_select(update: Update, context: ContextTypes.DEFAULT_TYPE):
    field = update.message.text.lower()
    if field not in ["nom", "intro", "objectif", "durée"]:
        await update.message.reply_text("❗ Choix invalide. (nom, intro, objectif, durée)")
        return MODIFY_SELECT
    context.user_data["field"] = field
    await update.message.reply_text(f"✏️ Envoie la nouvelle valeur pour *{field}*.", parse_mode="Markdown")
    return MODIFY_INPUT

async def modify_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def delete_crossover(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.chat.type != "private" or update.message.from_user.username != CREATOR_USERNAME:
        await update.message.reply_text("❗ Commande réservée au créateur, en privé.")
        return
